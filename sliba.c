...
LISP array_gc_mark(LISP ptr) {
    ...
    if TYPEP(ptr, tc_lisp_array) {
        for (j = 0; j < ptr->storage_as.lisp_array.dim; ++j) {
            gc_mark(ptr->storage_as.lisp_array.data[j], -1L, -1L);
        }
    }
    return (NIL);
}
...
void rfs_ungetc(unsigned char c, unsigned char **p) { *p = *p - 1; }
...
void err1_aset1(LISP i) { err("index to aset too large", i); }

void err2_aset1(LISP v) { err("bad value to store in array", v); }
...
LISP sxhash(LISP obj, LISP n) { return (flocons(c_sxhash(obj, FLONUMP(n) ? (long) FLONM(n) : 10000))); }
...
void put_long(long i, FILE *f) { fwrite(&i, sizeof(long), 1, f); }
